...
async def spawn_agent(agent_type: str, task: str) -> dict:
    ...
    `agent_type` is 'researcher' (finds & summarizes information) or 'planner' (breaks a
    goal into concrete tasks). Returns the new run id — the agent runs on its own and
    ...
    if agent_type == "ada" or agent_type not in registry.SPECS:
        return {"error": f"unknown agent_type '{agent_type}'. Options: researcher, planner"}
