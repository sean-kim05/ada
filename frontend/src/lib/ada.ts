...
// The cached daily brief (pre-generated by the scheduler) — instant, no waiting.
export interface GmailBrief {
  authorized: boolean;
  needs_auth?: boolean;
  cached: { generated_at: string; unread: number; important: GmailMsg[]; brief: string } | null;
}
export async function getGmailBrief(): Promise<GmailBrief> {
  const r = await fetch(`${API}/api/gmail/brief`);
  return r.json();
}
